...
function shapeText(text: string,
                   ...
                   writingMode: 1 | 2,
                   glyphs: {[number]: ?StyleGlyph}): Shaping | false {
    ...
    for (let line of lines) {
        ...
        for (let i = 0; i < line.length; i++) {
            ...
            if (!scriptDetection.charHasUprightVerticalOrientation(codePoint) || writingMode === WritingMode.horizontal) {
                positionedGlyphs.push({glyph: codePoint, x, y, angle: 0});
                x += glyph.metrics.advance + spacing;
            } else {
                positionedGlyphs.push({glyph: codePoint, x, y: 0, angle: -Math.PI / 2});
...
function shapeIcon(image: ImagePosition, iconOffset: [number, number]): PositionedIcon {
